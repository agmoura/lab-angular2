...
public abstract class BaseController<TEntity extends IEntity> {
    ...
    @Transactional
    @PatchMapping("{id}")
    public TEntity patch(@PathVariable String id, @RequestBody String entityData)  {

        TEntity entity, currentEntity = this.get(id);

        try {
            entity = new ObjectMapper().readerForUpdating(currentEntity).readValue(entityData);
        } catch (IOException ex) {
            throw new ApplicationException("Erro ao deserializar '" + entityData + "' na entidade '" + entityClass.getSimpleName() + "'");
        }
